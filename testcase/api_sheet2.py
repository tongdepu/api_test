...
sheet = ReadConfig()
sheet_data = sheet.get_sheet("sheet2")  # 需要测试哪个sheet就改成对应sheet
...
class TestAPI (unittest.TestCase):   # 类必须以Test开头，继承TestCase
    ...
    # 测试用例
    @data(*test_data)
    def test_api(self,data_itme):     # 测试用例脚本方法也必须以test开头
        headers = {
        ...
        if str.lower(data_itme['method']) == 'get':     # str.lower() 将字符串中所有的大写转换成小写
            if data_itme['params'] != '':       # 如果excel表格中的params单元格不为空，则传参
        ...
        try:
            ...
            write.write_result(os.path.dirname(os.path.abspath(".")) + '\\test_data\\test.xlsx',int(data_itme['id']),str(response_data),str(test_res),0)
